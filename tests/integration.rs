...
fn create_test_db() -> (tempfile::NamedTempFile, LocalDb) {
    ...
    conn.execute_batch("
        ...
        ('介護職', '施設A', '特別養護老人ホーム', '群馬県', '高崎市', '正社員', '月給', 220000, 280000, 200000, 'あり(年2回)', 110, 10000, '住宅手当', '介護福祉士', 36.32, 139.00),
        ('介護職', '施設B', '特別養護老人ホーム', '群馬県', '高崎市', '正社員', '月給', 200000, 260000, 180000, 'あり(年2回)', 115, 8000, '', '介護職員初任者研修', 36.33, 139.01),
        ('介護職', '施設C', 'デイサービス', '群馬県', '高崎市', '正社員', '月給', 190000, 250000, 170000, 'あり', 120, 5000, '通勤手当', '', 36.34, 139.02),
        ('介護職', '施設D', 'デイサービス', '群馬県', '高崎市', 'パート', '時給', 1100, 1300, 0, '', 0, 0, '', '', 36.31, 138.99),
        ('介護職', '施設E', 'グループホーム', '群馬県', '高崎市', '正社員', '月給', 210000, 270000, 190000, 'あり(年2回)', 108, 10000, '夜勤手当', '介護福祉士', 36.35, 139.03),
        ('介護職', '施設F', 'グループホーム', '群馬県', '高崎市', '正社員', '月給', 230000, 290000, 210000, 'あり(年3回)', 112, 15000, '資格手当', '介護福祉士', 36.30, 138.98),
        ('介護職', '施設G', '訪問介護', '群馬県', '高崎市', 'パート', '時給', 1200, 1500, 0, '', 0, 0, '', '', 36.29, 138.97);

        -- 群馬県前橋市のデータ（3件）
        INSERT INTO job_postings (job_type, facility_name, facility_type, prefecture, municipality, employment_type, salary_type, salary_min, salary_max, base_salary, bonus, annual_holidays, qualification_allowance, other_allowances, requirements, latitude, longitude) VALUES
        ('介護職', '施設H', '特別養護老人ホーム', '群馬県', '前橋市', '正社員', '月給', 200000, 260000, 180000, 'あり', 110, 8000, '', '介護福祉士', 36.39, 139.06),
        ('介護職', '施設I', 'デイサービス', '群馬県', '前橋市', 'パート', '時給', 1000, 1200, 0, '', 0, 0, '', '', 36.40, 139.07),
        ('介護職', '施設J', 'グループホーム', '群馬県', '前橋市', '正社員', '月給', 215000, 275000, 195000, 'あり(年2回)', 115, 10000, '通勤手当', '介護職員初任者研修', 36.38, 139.05);

        -- 東京都の追加データ（3件）
        INSERT INTO job_postings (job_type, facility_name, facility_type, prefecture, municipality, employment_type, salary_type, salary_min, salary_max, base_salary, bonus, annual_holidays, qualification_allowance, other_allowances, requirements, latitude, longitude) VALUES
        ('介護職', '施設K', '特別養護老人ホーム', '東京都', '新宿区', '正社員', '月給', 250000, 320000, 230000, 'あり(年2回)', 120, 15000, '住宅手当', '介護福祉士', 35.69, 139.70),
        ('介護職', '施設L', 'デイサービス', '東京都', '渋谷区', '正社員', '月給', 240000, 310000, 220000, 'あり', 118, 10000, '', '介護福祉士', 35.66, 139.70),
        ('看護師', '施設M', '病院', '東京都', '新宿区', '正社員', '月給', 300000, 400000, 280000, 'あり(年2回)', 125, 20000, '夜勤手当', '正看護師', 35.69, 139.69);
...
async fn test_competitive_salary_stats() {
    ...
    let resp = app
        ...
        .unwrap();
    let body = body_string(resp.into_body()).await;
    // 正社員の給与（190,000〜230,000）が含まれるか確認
...
/// テスト37: 雇用形態フィルタ employment_type=正社員
#[tokio::test]
async fn test_competitive_filter_employment_type() {
    ...
    let body = body_string(resp.into_body()).await;
    // パートは含まれないはず
    assert!(!body.contains("施設D") || body.contains("正社員"));
}
